...
def data_save_stock_data(df):
    ...
    try:
        ...
        # 插入stocks表（如果不存在）
        cursor.execute('INSERT OR IGNORE INTO stocks (ts_code, name, concept) VALUES (?, ?, ?)', (ts_code, name, concept))
        ...
        for _, row in df_sorted.iterrows():
            ...
            # 插入stock_data
            cursor.execute('INSERT OR REPLACE INTO stock_data (ts_code, trade_date, open, high, low, close, pre_close, volume, amount, turnover_rate, pe, circ_mv, concept) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (ts_code, trade_date, open, high, low, close, pre_close, volume, amount, turnover_rate, pe, circ_mv, concept))
...
def data_download_single_stock_data(contextInfo, ts_code, start_date, end_date):
    ...
    try:
        ...
        try:
            ...
            print(f'all_sectors={all_sectors}')
            # 用于存储该股票所属的板块
            belonged_sectors = []
            ...
            for sector_name in all_sectors:
                ...
                print(f'sector_name={sector_name}, stock_list={stock_list}')
                # 检查目标股票是否在当前板块的成分股列表中
                if ts_code in stock_list:
                    belonged_sectors.append(sector_name)

            concept = ','.join(belonged_sectors) if belonged_sectors else None
        except Exception as e:
            concept = None
            print(f'Warning! 获取 {ts_code} 的概念板块信息失败 {e}')
        df['concept'] = concept
        print(f'{ts_code} 所属概念板块: {concept}')
        # 获取换手率
        ...
        # 选择需要的列
        df = df[['ts_code', 'name', 'trade_date', 'open', 'high', 'low', 'close', 'pre_close', 'volume', 'amount', 'turnover_rate', 'pe', 'circ_mv', 'concept']]
...
def data_load_stock(stock_code, start_date='20150101'):
    ...
    columns = ['股票代码', '股票名称', '日期', '开盘', '收盘', '前收盘', '最高', '最低', '成交量', '成交额', '换手率', '市盈率', '流通市值', '概念']
    try:
        conn = sqlite3.connect('C:/a/trade/量化/中信证券/code/stock_data.db')
        cursor = conn.cursor()
        ...
        # 查询指定股票数据
        cursor.execute('''
            SELECT d.ts_code, d.trade_date, d.open, d.high, d.low, d.close, d.pre_close, d.volume, d.amount, d.turnover_rate, d.pe, d.circ_mv, s.name, s.concept
            FROM stocks s
        ...
        # 转换为DataFrame
        df = pd.DataFrame(rows, columns=['ts_code', 'trade_date', 'open', 'high', 'low', 'close', 'pre_close', 'volume', 'amount', 'turnover_rate', 'pe', 'circ_mv', 'name', 'concept'])
        # 重命名列为中文
