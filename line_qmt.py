...
def init_load_codes_in_position(contextInfo):
	...
	for dt in positions:
		...
		if code not in T.codes_in_position:
			...
			T.codes_in_position[code]['shares'] = round(dt.m_nVolume, 0)
			T.codes_in_position[code]['price'] = round(dt.m_dOpenPrice, 2)
			T.codes_in_position[code]['market_value'] = round(dt.m_dMarketValue, 0)
...
def on_timer(contextInfo):
	...
	# 	ticks = contextInfo.get_full_tick(sorted(set(T.codes.keys())))
...
def after_init(contextInfo):
	...
	# trade_buy_stock_by_volume(contextInfo, sorted(T.codes.keys())[2], 100, 'test trade_buy_stock_by_volume()')
...
def trade_refine_codes1(contextInfo):
	...
	for code in sorted(set(T.codes.keys())):
		...
		# 	continue
		# log(f'trade_refine_codes(): code={code}, name={T.codes[code]["name"]}, closes[-1]={closes[-1]}')
	...
	for code in sorted(set(T.codes.keys())):
		...
		if contextInfo.do_back_test:
			...
			if market_data_last_price[code].empty:
				...
				continue
			current_low = round(market_data_last_price[code]['low'][0], 2)
			current_high = round(market_data_last_price[code]['high'][0], 2)
			current = round(market_data_last_price[code]['close'][0], 2)
			if current == 0:
		...
		if T.codes[code]['high'] is None:
			...
			T.codes[code]['high_is_changed'] = True
		# 获取120日的成交额
		market_data_120 = contextInfo.get_market_data_ex(['amount', 'close', 'low', 'open', 'high'], [code], period='1d', end_time=T.CURRENT_DATE, count=30, dividend_type='front', fill_data=False, subscribe=True)
		...
		if buy_date and buy_date in lows.index:
			...
			local_min = 0
		# 计算成交量相对量比
		rolling_max = pd.Series(amounts).rolling(window=10).max().values
		rolling_min = pd.Series(amounts).rolling(window=10).min().values
		...
		# current = current_low
		# if T.codes[code]['type'] in [None] and T.codes[code]['last_type'] in [None, 'SELL_AT_LOCAL_MAX', 'SELL_AT_TIMEOUT', 'SELL_AT_STEP_0'] and amount_ratios[-2] == 0 and rates[-2] < 8 and rates[-2] > -8 and amounts[-2] < avg_amount_120 and amounts[-3] < avg_amount_120 and amounts[-4] < avg_amount_120 and amounts[-5] < avg_amount_120 and amounts[-6] < avg_amount_120:
		# 	T.codes[code]['type'] = 'BUY_AT_LOCAL_MIN'
		# 	T.codes[code]['price'] = current
		# 	log(f'{current_time} {T.codes[code]["type"]}: {code} {T.codes[code]["name"]}, current={current:.2f}, opens[-1]={opens[-1]:.2f}, amounts[-1]={amounts[-1]:.1f}, avg_amount_120={avg_amount_120:.1f}, rates[-1]={rates[-1]:.2f}, rates[-2]={rates[-2]:.2f}, rates[-3]={rates[-3]:.2f}, amount_ratios[-1]={amount_ratios[-1]:.2f}, amount_ratios[-2]={amount_ratios[-2]:.2f}, amount_ratios[-3]={amount_ratios[-3]:.2f}, closes[-2]={closes[-2]:.2f}, closes[-3]={closes[-3]:.2f}, lows[-2]={lows[-2]:.2f}, lows[-3]={lows[-3]:.2f}, local_max={local_max:.2f}, local_min={local_min:.2f}')
		# 	shares = trade_buy_stock_by_amount(contextInfo, code, T.BUY_AMOUNT, T.codes[code]['price'], T.codes[code]['type'])
		...
		# current = current_low
		if T.codes[code]['type'] in [None] and T.codes[code]['last_type'] in ['BUY_AT_LOCAL_MIN', 'SELL_AT_STEP_1'] and amount_ratios[-2] == 0 and rates[-2] < 8 and rates[-2] > -8 and amounts[-2] < avg_amount_120 and trade_get_last_buy_price(contextInfo, code) is not None and current < 0.93 * trade_get_last_buy_price(contextInfo, code):
		...
		continue
		current = current_high
		# 卖出: 当日出现高于BUY_AT_STEP_x买入价的1.08倍时, 卖出此份股票. buy_price要从T.codes[code]['records']里枚举, 还包括当日买入的T.codes[code]['price']. 卖出时, 用SELL_AT_STEP_0标记
...
def trade_query_info(contextInfo):
	...
	for o in orders:
		...
		if order_date >= N_days_ago:
			...
			log(f'trade_query_info(): {o.m_strInstrumentID}.{o.m_strExchangeID} {o.m_strInstrumentName}, 委托方向: {direction}', f' 数量: {o.m_nVolumeTotalOriginal}, 成交均价: {o.m_dTradedPrice:.2f} 元, 成交数量: {o.m_nVolumeTraded}, 成交金额: {o.m_dTradeAmount:.0f} 元, 时间: {o.m_strInsertDate}T{o.m_strInsertTime.zfill(6)}')
	...
	for dt in deals:
		...
		if deal_date >= N_days_ago:
			...
			log(f'trade_query_info(): {dt.m_strInstrumentID}.{dt.m_strExchangeID} {dt.m_strInstrumentName}, 成交方向: {direction}', f' 价格: {dt.m_dPrice:.2f}, 数量: {dt.m_nVolume}, 金额: {dt.m_dTradeAmount:.0f} 元, 时间: {dt.m_strTradeDate}T{dt.m_strTradeTime.zfill(6)}')
...
def trade_sell_stock_by_shares(contextInfo, code, shares, last_price, comment):
	shares_in_stock = 0
	positions = get_trade_detail_data(T.accountid, 'stock', 'position')
	for dt in positions:
		if f"{dt.m_strInstrumentID}.{dt.m_strExchangeID}" != code:
...
def trade_buy_stock_at_up_stop_price_by_amount(contextInfo, code, buy_amount, comment):
	...
	log(f'trade_buy_stock_at_up_stop_price_by_amount(): {code} {T.codes[code]["name"]} 以涨停价{up_stop_price:.2f}买入 {volume}手金额 {buy_amount:.0f} 元')
...
def trade_buy_stock_at_up_stop_price_by_volume(contextInfo, code, volume, comment):
	...
	# 使用passorder进行指定价买入，prType=11，price=up_stop_price
	passorder(T.opType_buy, T.orderType_volume, T.accountid, code, T.prType_designated, up_stop_price, volume, T.strategyName, T.quickTrade, comment, contextInfo)
	log(f'trade_buy_stock_at_up_stop_price_by_volume(): {code} {T.codes[code]["name"]} 以涨停价{up_stop_price:.2f}买入 {volume} 股，预计成交金额 {buy_amount:.0f} 元')
...
def trade_buy_stock_by_amount(contextInfo, code, buy_amount, last_price, comment):
	log(f'trade_buy_stock_by_amount(): {code} {T.codes[code]["name"]}, buy_amount={buy_amount:.2f}元, last_price={last_price}')
	...
	if last_price == 0 or last_price is None:
		log(f'trade_buy_stock_by_amount(): Error! Invalid last_price! last_price={last_price}!')
		return 0
	# 计算买入股数
	...
	log(f'trade_buy_stock_by_amount(): {code} {T.codes[code]["name"]} 指定价格{last_price:.2f} 买入 {volume} 股，预计成交金额 {actual_buy_amount:.0f} 元')
	return volume
...
def trade_buy_stock_by_volume(contextInfo, code, volume, comment):
	...
	log(f'trade_buy_stock_by_volume(): {code} {T.codes[code]["name"]} 市价买入 {volume} 股，预计成交金额 {trade_amount:.0f} 元')
...
def order_callback(contextInfo, orderInfo):
	...
	if orderInfo.m_nOrderStatus == 56:  # 已成
		log(f'order_callback(): 委托已全部成交 - 股票: {code} {name}, 委托ID: {orderInfo.m_strOrderSysID}, 成交数量: {orderInfo.m_nVolumeTraded}, 成交均价: {orderInfo.m_dTradedPrice:.2f}, 成交金额: {orderInfo.m_dTradeAmount:.0f} 元, m_nDirection={orderInfo.m_nDirection}, m_strOptName={orderInfo.m_strOptName}')
		...
		log(f'order_callback(): 委托部分成交 - 股票: {code} {name}, 委托ID: {orderInfo.m_strOrderSysID}, 已成交数量: {orderInfo.m_nVolumeTraded}, 剩余数量: {orderInfo.m_nVolumeTotal}, 成交金额: {orderInfo.m_dTradeAmount:.0f} 元, m_nDirection={orderInfo.m_nDirection}, m_strOptName={orderInfo.m_strOptName}')
...
def deal_callback(contextInfo, dealInfo):
	...
	# 检查成交结果并记录
	# log(f'deal_callback(): 成交确认 - 股票: {code} {name}, 成交ID: {dealInfo.m_strTradeID}, 成交价格: {dealInfo.m_dPrice:.2f}, 成交数量: {dealInfo.m_nVolume}, 成交金额: {dealInfo.m_dTradeAmount:.0f}, 买卖方向: {dealInfo.m_nDirection}')
	# 可以在这里添加更多逻辑，如更新全局变量、发送通知等
	# 例如，检查是否为买入或卖出，并更新持仓统计
	log(f'deal_callback(): {code} {name}, 成交ID: {dealInfo.m_strTradeID}, 成交价格: {dealInfo.m_dPrice:.2f}, 成交数量: {dealInfo.m_nVolume}, 成交金额: {dealInfo.m_dTradeAmount:.0f} 元, m_strOptName={dealInfo.m_strOptName}')
...
def position_callback(contextInfo, positionInfo):
	...
	if code in T.codes:
		...
		log(f'position_callback(): Warning! {code} {name} is not in T.codes!')
